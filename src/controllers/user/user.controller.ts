...
class CompanyController {
    ...
    public async register_user(@Body() body: IRegisterUserRequest, @Request() auth_company: _AuthCompany): Promise<IUserResponse> {
        try {
            ...
            const isExist = await UserRepo.user_repo_find('email', body.email);
            if (isExist) {
                result = {
                ...
                const data = await UserRepo.user_repo_register({ ...body, auth_company });
    ...
    public async login_user(@Body() body: ILoginUserRequest): Promise<IUserResponse> {
        try {
            const userData = <IUser & IUserLoginResponse>await UserRepo.user_repo_login(body)
    ...
    public async login_generic(@Body() body: any): Promise<IUserResponse> {
        try {
            ...
            if (!userData) result = {
                ...
                const { user_name, first_name, last_name, email, _id, namespace, socket_id, company_id } = userData;
                const user_id = _id;
                const authToken = generate_jwt({ user_id, namespace, user_name, first_name, last_name, email, socket_id, company_id }, `${process.env.USER_JWT_SECRET}`, `${process.env.USER_JWT_EXPIRES_IN}`)
                result = {
                    message: `User logged in successfully!!`,
    ...
    public async update_user_profile(@Body() body: IUserProfileRequest, @Request() auth_user: _AuthUser): Promise<IUserResponse> {
        try {
            let result: IUserResponse = {};
            const isUserExist = await UserRepo.user_repo_find("_id", auth_user.user_id);
            if (!isUserExist) {
                result = {
                    message: `User not found!!`,
                    statusCode: CODES.API.STATUS_NOT_FOUND,
                ...
                await UserRepo.user_repo_update_profile(body, auth_user);
                result = {
