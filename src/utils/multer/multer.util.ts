...
const storage = multer.diskStorage({
    destination: (
        ...
        const isDirExist = fs.existsSync("uploads");
        if (!isDirExist) {
            fs.mkdir("uploads", { recursive: true }, (err: NodeJS.ErrnoException | null, folder_path: string | undefined) => {
                if (err) next(err, "uploads");
                next(null, folder_path as string);
            })
        } else next(null, "uploads");
    },
